...
(function($){
	 ...
	 *            height 样式高度
	 */
	JRJ.ui.ZQTPassword=function(selector,accountId,action,bankNo,width,height){
	...
	JRJ.ui.ZQTPassword.prototype={
			...
			init:function(){
				...
				$.ajax({
						type : "POST",
						url : _this.url.replace("#1", _this.accountId),
						data:{
							"action":_this.action,
							"bankNo" :_this.bankNo
						},
			...
			check:function(callback){
				...
				if(this.type==1){
					...
					case AC.TRADE:
					case AC.CANCEL:
						var password =$(_this.selector+" .ZQT_password_AX").get(0).PwdStr;
						_this._mac=$(_this.selector+" .ZQT_password_AX").get(0).MacStr;
						if(password ==undefined){
							_this.Ifm_callback(-99);//密码控件未安装
						}else if( password==""){
							_this.Ifm_callback(-100);//交易密码不能为空
						}else{
							this.values[0]=password;
					...
					case AC.QUERY:
						...
						if(fundPassword.data("password") ==1){
							var password =$(_this.selector+" .fundPasswordObj").get(0).PwdStr;
							_this._mac=$(_this.selector+" .fundPasswordObj").get(0).MacStr;
							if (password ==undefined){
								_this.Ifm_callback(-99);//密码控件未安装
								return;
							}else if(password==""){
								_this.values["fundPassword"]="";
								_this.Ifm_callback(-101);//资金密码不能为空
								return ;
							}else{
								_this.values["fundPassword"]=password;
							}
						}
						if(bankPassword.data("password") ==1){
							var password =$(_this.selector+" .bankPasswordObj").get(0).PwdStr;
							_this._mac=$(_this.selector+" .bankPasswordObj").get(0).MacStr;
							if (password ==undefined ){
								_this.Ifm_callback(-99);//密码控件未安装
								return;
							}else if(password==""){
								_this.values["bankPassword"]="";
								_this.Ifm_callback(-102);//银行密码不能为空
								return ;
							}else{
								_this.values["bankPassword"]=password;
							}
						}
						if(tradePassword.data("password") ==1){
							var password =$(_this.selector+" .tradePasswordObj").get(0).PwdStr;
							_this._mac=$(_this.selector+" .tradePasswordObj").get(0).MacStr;
							if (password ==undefined ){
								_this.Ifm_callback(-99);//密码控件未安装
								return;
							}else if( password==""){
								_this.values["tradePassword"]="";
								_this.Ifm_callback(-100);//交易密码不能为空
								return ;
							}else{
								_this.values["tradePassword"]=password;
