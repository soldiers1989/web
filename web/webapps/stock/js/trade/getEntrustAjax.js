...
nextUrl="/stock/"+_accountId+"/getEntrustPage.jspa";
	updatePageStatus();
	//自定义--日期输入控制
	$("#seltitle").hide();
	$("#cal-1").hide();
	...
	$('input:radio').click(function (){ 
		if($('input[name="Radio1"]:checked').val()!="custom"){
			...
			if(selectType=="today"){//当日委托记录
				selUrl="/stock/"+_accountId+"/getEntrustPage.jspa";
			}else{
				selUrl="/stock/"+_accountId+"/getHistoryEntrustPage.jspa";
			}
	        ...
	        var startdate=$("#cal-1").val().replace(/-/g,"");
			var enddate =$("#cal-2").val().replace(/-/g,"");
			var selUrl="/stock/"+_accountId+"/getHistoryEntrustPage.jspa";
			var selectType =$('input[name="Radio1"]:checked').val();
			getContent(selUrl,1,selectType,startdate,enddate);
		}
	}); 
	...
	$("#selectBtn").click(function(){
		...
		if(selectType=="custom"){
			...
			selUrl="/stock/"+_accountId+"/getHistoryEntrustPage.jspa";
		}else if(selectType=="today"){//当日委托记录
			selUrl="/stock/"+_accountId+"/getEntrustPage.jspa";
		}else{
			selUrl="/stock/"+_accountId+"/getHistoryEntrustPage.jspa";
		}
...
//				var dateCount = (_end - _begin)/86400000;
				...
				nextUrl="/stock/"+_accountId+"/getHistoryEntrustPage.jspa";
